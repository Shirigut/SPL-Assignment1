...
public abstract class MicroService implements Runnable {
    //static fields
    private static MessageBusImpl messageBus = MessageBusImpl.getInstance();
    protected static Diary diary = Diary.getInstance();
    //local fields
    //saves the message types with their personal callbacks
    ...
    public final void run() {
        ...
        initialize();
        Message message;
        //runs until the microservice terminates
        while (!terminationCondition) {
            try {
                message = messageBus.awaitMessage(this); //takes the next message from the queue
                callbacks.get(message.getClass()).call(message); //call the right callback from the callbacks hashmap
            } catch (InterruptedException ex) {
                System.out.print("No new messages");
            }
        }
    }
    //every microservice override this method with his personal termination time
    protected void setTerminationTime () {
    }

    protected void subscribeToTerminate() {
        //subscribes to termination broadcast with the relevant callback
        subscribeBroadcast(TerminationBroadcast.class, (TerminationBroadcast terminate) -> {
            terminate();
            setTerminationTime();
        });
    }
